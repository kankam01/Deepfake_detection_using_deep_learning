...
def plot_heat_map(i, model, img, path='./', video_file_name=''):
    ...
    image_name = os.path.join(settings.BASE_DIR, 'uploaded_images', heatmap_name)
...
def get_accurate_model(sequence_length):
    list_models = glob.glob(os.path.join(settings.BASE_DIR, "models", "*.pt"))
    model_name = []
    sequence_model = []
    final_model = ""
    ...
    return os.path.join(settings.BASE_DIR, "models", final_model)
...
def login_view(request):
    if request.method == 'POST':
        ...
        # Simple authentication (replace with Django's auth system for production)
        if username == 'user' and password == 'password':
            request.session['is_authenticated'] = True
            return redirect('ml_app:selection')
        else:
...
def selection_view(request):
    if not request.session.get('is_authenticated', False):
        return redirect('login')  # Redirect to root URL (login page)
    ...
    if request.method == 'GET':
        ...
        for model in models:
            try:
                seq = model.split("_")[3]
                acc = model.split("_")[1]
                choices.append((seq, f"{seq} Frames ({acc}% Accuracy)"))
            except IndexError:
                pass
        video_upload_form.fields['sequence_length'].choices = choices
        ...
        if video_upload_form.is_valid():
            ...
            video_path = os.path.join(settings.BASE_DIR, 'uploaded_videos', saved_video_file)
            with open(video_path, 'wb') as vFile:
                shutil.copyfileobj(video_file, vFile)
            request.session['file_name'] = video_path
...
def predict_page(request):
    ...
    if request.method == "GET":
        ...
        sequence_length = request.session.get('sequence_length', 100)
        ...
        print(f"Attempting to load model from: {path_to_model}")
        
        if not path_to_model or not os.path.exists(path_to_model):
            return render(request, predict_template_name, {
                "error": f"No model found for sequence length {sequence_length} or model file missing."
            })
        ...
        for i in range(min(sequence_length, len(frames))):
            ...
            image_path = os.path.join(settings.BASE_DIR, 'uploaded_images', image_name)
        ...
        try:
            ...
            for i in range(len(path_to_videos)):
                ...
                # Uncomment to enable heatmap generation
            ...
            context = {
                ...
                'models_location': os.path.join(settings.BASE_DIR, 'models'),
                'output': output,
